...
- `page_kind`: choose ONE of clinical, imaging, pathology, functional, admin, signature_only, empty.
  ...
  - functional: STANDALONE capacity and demands instruments - functional abilities or capacity evaluations, job descriptions, physical demands analyses, and screening or outcome tools completed as documents in their own right. A treating clinician's report form is NOT functional merely because it states restrictions: an attending physician statement, a physician's initial report form, or an insurer's medical form is `clinical`, because its restrictions are one section of a clinical report rather than the purpose of the document. Ask what the document IS, not which fields it contains.
  ...
  - AN ADMINISTRATIVE SECTION IS NOT AN ADMINISTRATIVE PAGE. Clinical forms routinely open with an identification and authorization block - insurer name, member details, SIN, "I hereby authorize the release of any information", a member signature - before the medical sections begin. That block is part of the form, not a consent document. The `admin` kinds above mean a whole document that is nothing but a cover, a bill, a consent, or a fax header. Judge the WHOLE page: if any part of it carries clinical or functional content, or if the page names itself a part of a larger form ("Part 2 - to be completed by Member", "Part 3 to 8 - to be completed by a Medical Doctor") or prints a page marker such as "Page 1 of 5", it is NOT admin - classify it by its medical content.
  - empty: blank or near-blank page (logos, page numbers only).
  - clinical: anything else medical (consults, notes, referrals, hospital records, telephone interviews, case-management notes). IMPORTANT: Member/patient-filled claim forms, disability benefit statements, or insurance application forms that contain symptom descriptions, diagnosis fields, or medical history narratives are CLINICAL, not admin — even though they are forms. Only classify as admin if the page contains NO clinical/medical content at all. A page whose top half is identification and authorization and whose lower half states a diagnosis, an onset date, or a mechanism of injury is CLINICAL: the diagnosis decides it, not the boilerplate above it.
